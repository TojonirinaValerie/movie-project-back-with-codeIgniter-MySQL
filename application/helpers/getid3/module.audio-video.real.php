...
class getid3_real extends getid3_handler
{
			...
			switch ($ChunkName) {
				...
				case 'MDPR': // Media Properties Header
					...
					if ($thisfile_real_chunks_currentchunk['object_version'] == 0) {
						...
						$thisfile_real_chunks_currentchunk['mimeType_size']     = getid3_lib::BigEndian2Int(substr($ChunkData, $offset, 1));
						$offset += 1;
						$thisfile_real_chunks_currentchunk['mimeType']          = substr($ChunkData, $offset, $thisfile_real_chunks_currentchunk['mimeType_size']);
						$offset += $thisfile_real_chunks_currentchunk['mimeType_size'];
						...
						switch ($thisfile_real_chunks_currentchunk['mimeType']) {
	...
	public function ParseOldRAheader($OldRAheaderData, &$ParsedArray) {
		...
		if ($ParsedArray['version1'] < 3) {
			...
			$ParsedArray['fileSize']        = getid3_lib::BigEndian2Int(substr($OldRAheaderData, 12, 4));
