...
class getid3_nsv extends getid3_handler
{
	...
	public function getNSVfHeaderFilepointer($fileoffset, $getTOCoffsets=false) {
		...
		$offset += 4;
		$info['nsv']['NSVf']['fileSize']     = getid3_lib::LittleEndian2Int(substr($NSVfheader, $offset, 4));
		$offset += 4;

		if ($info['nsv']['NSVf']['fileSize'] > $info['avdataend']) {
			$this->warning('truncated file - NSVf header indicates '.$info['nsv']['NSVf']['fileSize'].' bytes, file actually '.$info['avdataend'].' bytes');
		}
		...
		$info['bitrate']          = ($info['nsv']['NSVf']['fileSize'] * 8) / $info['playtime_seconds'];
